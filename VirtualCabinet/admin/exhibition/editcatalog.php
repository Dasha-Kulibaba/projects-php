...
$query = "SELECT documents.id,documents.title, documents.link, users.name,documents.create_date  FROM `documents`   JOIN `users` ON documents.author_id=users.id WHERE `author_id`=" . $_SESSION['email'];
			...
			<div class="autorize__content">
				...
				if ($res->num_rows > 0) {
					while ($item = mysqli_fetch_assoc($res)) {
						...
						<a href="../exhibition/edit.php?id=' . $item['id'] . '" class="admin__icon admin__edit">
