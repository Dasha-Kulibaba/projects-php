...
async function fetchData() {
  const response = await fetch("../../php/young.php");
...
async function CreateBlock(filter) {
  ...
  if (admin) {
    ...
    button_create.href = "../admin/young/create.php";
...
function CreateItem(id, title, type, filetype, admin, link) {
  ...
  if (admin) {
    ...
    edit.href = "../admin/young/edit.php?id=" + id;
  ...
  p_subtitle.textContent = young_types[type];
...
const check = document.querySelectorAll(".checkbox__input");
check.forEach((item) => {
