...
const calendar = document.getElementById("hidden_calendar");
const months = [
  "Січень",
  "Лютий",
  "Березень",
...
async function createCalendar(month, year) {
  ...
  document.getElementById("hidden_month_name").textContent =
    months[currentMonth];
}

const button = document.getElementById("planner");
const calendar_body = document.getElementById("hidden__calendar");
const close = document.getElementById("hidden_close");

button.addEventListener("click", function () {
  if (!calendar_body.classList.contains("visible__calendar")) {
    calendar_body.classList.add("visible__calendar");
    document.body.style.height = "100vh";
    document.body.style.overflow = "hidden";
    createCalendar(today.getMonth(), today.getFullYear());
    close.addEventListener("click", function () {
      calendar_body.classList.remove("visible__calendar");
